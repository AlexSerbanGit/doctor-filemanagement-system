...
class DemoCron extends Command
{
    ...
    public function handle()
    {
        ...
        $filess =File::allFiles(dirname(dirname(public_path())).'/gallery');
        foreach (File::allFiles(dirname(dirname(public_path())).'/gallery') as $key=>$file) {
            ...
            if(strpos($strr[0], '_T') !== false){
                ...
                $document->save();
                $move = File::move(dirname(dirname(public_path())).'/gallery/'.$filess[$key]->getRelativePathname(), dirname(dirname(public_path())).'/gg/'.$document->id.'.'.File::extension($file));
